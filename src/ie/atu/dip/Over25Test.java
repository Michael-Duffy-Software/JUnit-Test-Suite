...
class Over25Test {
	...
	public void getCostTest(int accidents) {
		assertEquals(costResults[accidents],Over25.getCost(accidents));
	}
	...
	void printTimeTest() {
		for (int accidents = 0;accidents < 7; accidents++) {
			System.out.println(Over25.getCost(accidents));
		}
	}
	
	//Test That An Exception Is Thrown If A Negative Number of Accidents Is Given
	@Test
	void testValidateAccidentsExceptionNumberFormatException() {
		try {
			Over25.validateAccidents(throwException);
